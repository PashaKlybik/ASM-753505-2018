...
division:         ;äåëèì è ïðîâåðÿåì êàêîé ïîëó÷èëñÿ îñòàòîê
	mov cx, ax
	pop ax
	cwd
	idiv cx
...
outputSign proc    ; ïðîöåäóðà âûâîäà åñëè åñòü -
	push ax
	push dx
...
input proc        ; ââîä è ïðåîáðàçîâàíèå ñèìâîëà â ÷èñëî
	...
	push si
	push di
startInput:       ; ñìîòðèì öèôðà èëè - ïåðâîå
	xor di, di
	xor bx, bx
	xor si, si
	mov ah, 01h
...
cycl:
	mov ah, 01h
	int 21h
button:             ;ââîä(), óäàëåíèå ÷åðåç backspase è esc
	inc di
	cmp al, 13
	jz enter
	cmp al, 8
